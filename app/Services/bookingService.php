    ...
    public function updateBooking($allInput, $request, $booking)
    {
        ...
        switch($allInput['update-type'])
        {
            case 'create-documents';
                ...
                $bookingActions->addHistory($booking->id, 'Documents created');
                break;
            ...
            case 'send-documents';
                ...
                $sendEmailService->sendSendDocumentsEmail($booking, 'normal');
                $booking->update(['documents_sent' => 1]);
                $bookingActions->addHistory($booking->id, 'Documents sent');
                break;
            case 'send-reminder-documents';
                $sendEmailService = new sendEmailService();
                $sendEmailService->sendSendDocumentsEmail($booking, 'reminder');
                $bookingActions->addHistory($booking->id, 'Reminder email sent');
                break;
            case 'has-payed';
                $booking->update(['completed' => 1]);
                $bookingActions->addHistory($booking->id, 'Marked as payed');
                break;
            case 'cancel_booking';
